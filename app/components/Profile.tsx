  ...
  return (
    <div className="flex flex-col items-center text-center mb-8">
      ...
      <div className="flex flex-col gap-2 text-sm text-zinc-400">
        <a href={`mailto:${cv.basics.email}`} className="flex items-center justify-center gap-2 hover:text-white transition-colors">
          {Icons.email}
          {cv.basics.email}
        </a>
        <a href={`tel:${cv.basics.phone}`} className="flex items-center justify-center gap-2 hover:text-white transition-colors">
          {Icons.phone}
          {cv.basics.phone}
        </a>
      ...
      <div className="mt-4 flex gap-4">
        {cv.basics.profiles.map((profile, index) => (
          ...
          >
            {profile.network === "LinkedIn" ? Icons.linkedin : Icons.github}
          </a>
        ))}
      </div>
    </div>
